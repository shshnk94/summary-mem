"""Apply the pretrained OCEAN ridge pickle to `states_and_traits.msg_essays`.
...
The pickle was fit on Facebook status updates, not longitudinal diary essays -- a much bigger
domain shift than CANDOR's spoken-conversation transcripts. Whether it transfers here at all is
an empirical question the notebook's ground-truth comparison answers, not this script.

--group picks the prediction unit: user_id (one row per person).
"""
...
# `msg_essays` already uses DLATK's default `message` field, keyed by `message_id`.
MESSAGE_TABLE = "msg_essays"
MESSAGE_FIELD = "message"
MESSAGEID_FIELD = "message_id"

GROUPS = ["user_id"]  # DLATK's -c; a column of MESSAGE_TABLE
...
DLATK = TUTORIAL_DIR.parent / "dlatk" / "dlatkInterface.py"  # shared clone, tutorials/dlatk/
...
def prepare(group):
    ...
    The key is VARCHAR(50): DS4UD's `user_id` is `msg_essays.user_id varchar(50)`.
    """
    ...
    with engine.connect() as conn:
        ...
        conn.execute(
            text(
                ...
                f"  {group} VARCHAR(50) NOT NULL PRIMARY KEY, {columns}"
                f")"
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--group", choices=GROUPS, default="user_id",
